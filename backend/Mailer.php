...
class Mailer
{
    private static function send(string $to, string $subject, string $body): bool
    {
        ...
        if (!file_exists($configPath)) {
            $err = "'backend/mail_config.php' not found. Please create it from 'backend/mail_config_sample.php'.";
            self::logMail("ERROR", "System", $to, $subject, $err);
        ...
        self::logMail("ATTEMPT", $from, $to, $subject, "Starting SMTP delivery process");

        // 設定が初期状態のままの場合は送信エラーとする
        if ($config['password'] === 'your-app-password') {
            $err = "SMTP password is not configured in 'mail_config.php'.";
            self::logMail("ERROR", $from, $to, $subject, $err);
            error_log("Mailer Error: $err");
            return false;
        }
        ...
        foreach ($connections as $conn) {
            ...
            try {
                ...
                // 成功時：使用した接続方式をログに記録
                self::logMail("SUCCESS", $from, $to, $subject, "Sent via {$conn['secure']}:{$conn['port']}");
                error_log("Mailer: Email sent successfully via {$conn['secure']}:{$conn['port']}");
                return true;
            } catch (Exception $e) {
                $lastError = $mail->ErrorInfo;
                self::logMail("RETRY", $from, $to, $subject, "Failed with {$conn['secure']}:{$conn['port']} - $lastError");
        ...
        // すべての接続設定で失敗
        self::logMail("FAILURE", $from, $to, $subject, "All connection methods failed. Last error: $lastError");
        error_log("Mailer Error: All connection methods failed. Last error: $lastError");
        return false;
    }

    private static function logMail(string $status, string $from, string $to, string $subject, string $message)
    {
        $logDir = __DIR__ . '/../logs';
        if (!is_dir($logDir)) {
        ...
        $logEntry = "[$timestamp] [$status] FROM: $from | TO: $to | SUBJECT: $subject\n  MSG: $message\n";
