...
");
     ...
     * Replaces Socket.io / realtime-server for live message/DM/status updates.
     * Client connects once; PHP polls DB and pushes events to the browser.
     */
    private function streamEvents(): void
    {
        ...
        // Close the session so other PHP requests are not blocked
        ...
        // Starting cursor positions from query params (allow resume after reconnect)
        $lastMsgId  = max(0, (int)$this->getGet('last_msg_id', 0));
        $lastDmId   = max(0, (int)$this->getGet('last_dm_id', 0));
        $lastSeen   = max(0, (int)$this->getGet('last_status_ts', 0));

        // Seed from current max IDs if client sends 0 (first connect)
        if ($lastMsgId === 0) {
        ...
        $pollInterval  = 1500000; // 1.5 seconds in microseconds
        $heartbeatEvery = 20;     // send heartbeat every ~30 seconds (20 × 1.5s)
        $tick = 0;
        ...
        while (true) {
            ...
            // --- 1. New thread/group messages (キャッシュ判定付き) ---
            $globalLastMsgId = 0;
            ...
            // --- 2. New Direct Messages (キャッシュ判定付き) ---
            $globalLastDmId = 0;
            ...
            // --- 3. Heartbeat (keep connection alive, send current cursors) ---
