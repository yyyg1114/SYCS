...
class SignalingClient {
    ...
    async sendViaHttp(messageObj) {
        try {
            ...
            formData.append('receiver_id', messageObj.peer_id || 0);
            formData.append('type', messageObj.type);
            formData.append('content', JSON.stringify(messageObj.description || messageObj.candidate || {}));

            await fetch('api.php?action=send_signaling', {
                method: 'POST',
                body: formData
            });
        } catch (e) {
            console.error('[SignalingClient] HTTP send signaling failed:', e);
        }
    }
    ...
    startPollingFallback() {
        ...
        this.pollingInterval = setInterval(async () => {
            ...
            try {
                const res = await fetch(`api.php?action=get_signaling&room_id=${this.roomId}&last_id=${this.lastSignalingId}`);
                if (!res.ok) return;
                ...
                if (msgs && Array.isArray(msgs) && msgs.length > 0) {
                    for (const msg of msgs) {
                        ...
                        if (msg.type === 'offer') {
                            this.emitEvent('offer', { peerId: msg.sender_id, description: content });
                        } else if (msg.type === 'answer') {
                            this.emitEvent('answer', { peerId: msg.sender_id, description: content });
                        } else if (msg.type === 'candidate') {
                            this.emitEvent('ice_candidate', { peerId: msg.sender_id, candidate: content });
                        }
                    }
                }
            } catch (e) {
                console.error('[SignalingClient] Polling error:', e);
            }
        }, 2000);
    }
